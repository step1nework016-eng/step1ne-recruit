...
def build_prompt(ctx, skill_md):
    ...
    # 阿財誤判成「今年7月到職，兩個多月就想換」，被候選人當場抓包「你判讀看錯依德的日期」。
    # 根因是 prompt 從來沒告訴過阿財今天是幾號，年資/空窗期都是它自己瞎猜「現在是哪一年」
    # 算出來的。這裡直接寫死當下日期（每次組 prompt 都重新抓，本來就是即時的，不是寫死某天）。
    lines.append(f'\n【今天日期：{datetime.datetime.now().strftime("%Y-%m-%d")}】'
                 '算年資、空窗期、任職多久這類「距今多久」的問題，一律用這個日期回推，'
                 '不要憑印象猜現在是哪一年。')
    ...
    if job:
        ...
        # 平均薪資 X 起，不要用月薪 X 以上」互相矛盾——這裡是逐職缺注入、比 SKILL.md
        # 更貼近當下情境的內容，阿財實測會照這裡講，SKILL.md 那條完全沒生效。
        lines.append('     - 只有 salary_min、沒有 salary_max 時，一律講「平均薪資 X 起，實際依經驗面談決定」，'
                     '**絕對不要說「開的是 X 萬」「起薪 X 萬」或「月薪 X 以上」**。')
    ...
    if other_jobs:
        ...
        lines.append('  ② 主動加值建議（照真人顧問的做法）：真人顧問開始面談前會先看過履歷、'
                     '心裡先想好「我手上有哪些職缺可能適合這個人」，聊天過程中聽到對得上的地方，'
                     '才順勢切進去問。你也一樣——**開場前先看一遍下面這份清單跟這位候選人的'
                     '履歷／應徵表單，心裡先篩過一輪**：清單裡有沒有哪個職缺，看起來可能'
                     '也適合他？不用現在就講，記在心裡，等對話中聽到候選人講的經歷/技能跟'
                     '某個職缺的 must_skills／required_conditions 明顯對得上（不是模糊感覺，'
                     '是講得出具體是哪幾項符合）時，才自然切入。')
        lines.append('  切入的講法是**問句、邀請候選人回答**，不是單向報告完就結束：'
                     '「剛剛聽您這樣講，我們手邊剛好也有一個「（職缺名）」的職缺，'
                     '（用一兩句話簡單講這個職缺是做什麼的），聽起來跟您剛剛講的（具體符合項）'
                     '滿吻合的——不知道您對這個職缺有沒有興趣了解一下？」，然後**真的等候選人'
                     '回答，依照他的答案分兩條路走**：')
        lines.append('    - **候選人說有興趣**：順著這個職缺多聊幾句（這個職缺實際在做什麼、'
                     '他這方面的經驗細節），聊出更多資訊之後，再自然帶回目前這場真正要問的'
                     '職缺（硬性條件／專業題庫這些不能漏），不要讓面談就此整個轉到別的職缺去。')
        lines.append('    - **候選人說沒興趣，或答得含糊/想繼續聊原本的**：不要勉強、不要追問'
                     '為什麼沒興趣，順順地說一句「好的，那我們先繼續聊」帶回目前這場，'
                     '但這個「符合哪幾項、候選人當下沒興趣」的判斷還是要記錄下來給顧問參考——'
                     '沒興趣不代表不適合，可能只是候選人這次沒想清楚，顧問手上多一個線索。')
        lines.append('  **一場面談最多主動切入一個**，不要清單裡每個看起來沾得上邊的都拿出來講，'
                     '那會讓候選人覺得在亂槍打鳥。真的沒有明顯符合的，這場就不用主動提，不用硬湊。')
        lines.append('  ⚠️ 不管哪一種用法，只能講清單裡列出來的欄位，仍然**不要講客戶公司名稱**。'
                     '如果這場有做過②主動加值建議，請在輸出 JSON 的 note 欄位寫一句'
                     '「推薦了（職缺名），符合：xxx，缺失：xxx，候選人反應：有興趣/沒興趣/'
                     '沒明確表態」，讓顧問在報告裡看得到——不寫的話這段對話等於白做，'
                     '顧問完全不會知道。')
    ...
    if started:
        try:
            ...
            lines.append(
                ...
                f'⚠️ 2026-09-16 真實事故：候選人剛問完問題、明顯在等回覆，'
                f'阿財因為時間快到了，把「沒資料讓顧問講」＋摘要＋道別三則一次發送、'
                f'完全沒有照 Phase 6 規定分兩則等她回覆就把面談收掉——她其實還想聊。'
                f'**時間快到絕對不是跳過 Phase 6 兩則對話、等對方回覆這個步驟的理由**，'
                f'候選人剛問的問題沒回答完，一定要先回應（或明講「這幾個問題我留給顧問'
                f'幫你問清楚」），再照 Phase 6 順序：先講「時間差不多了」、等她回、'
                f'確認她真的沒有問題了，才進入道別那一則——不能把這些擠進同一輪一次發完。）')
        except Exception:
            pass
    # ── 讀不到履歷時的收尾交代 ──
