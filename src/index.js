  ...
  async fetch(request, env) {
    ...
    if (p.startsWith('/admin/')) {
      ...
      if (p.startsWith('/admin/jobs/') && p.endsWith('/content') && request.method === 'POST') {
        ...
        // ⚠️ 2026-09-02 再修：「存過檔」不代表存的是完整內容——如果之前只存過
        // 薪資這種單一欄位，jd_spec_json 就只有那幾個 key，工作內容／必要條件／
        // 為什麼選擇這個機會全部不在裡面。原本只看 hadSaved 會漏接這種「片段
        // 存檔」的職缺，這次改薪資一樣會把其他欄位清空。改成看內容夠不夠完整。
        const specLooksThin = !spec.duties && !spec.why && !spec.must;
        if (specLooksThin) {
          const scraped = await scrapeLiveJobPage(slug);
          ...
          // 已經存過的欄位代表顧問確實改過，優先權要高於重新掃到的內容，
          // 掃到的只用來補「從沒存過、也沒被掃過」的洞。
          if (scraped && scraped.spec) spec = Object.assign({}, scraped.spec, spec);
        }
        // ⚠️ 2026-09-02 加：不管上面的來源判斷有沒有漏洞，存檔前一律再檢查一次
        // 「這次存完內容會不會突然變超短」——這是最後一道防線，防的是還沒被
        // 發現的其他清空路徑，不只是這次修的那一種。
        const beforeLen = JSON.stringify(spec).length;
        ...
        const afterLen = JSON.stringify(spec).length;
        if (beforeLen > 500 && afterLen < beforeLen * 0.6 && !b.confirm_shrink) {
          return json(request, { ok: false, error: 'content_shrink',
            shrink: true, beforeLen, afterLen,
            message: `這次存檔後內容會從約 ${beforeLen} 字變成約 ${afterLen} 字，掉了一大截，看起來不像只是改個欄位。若確定要這樣存，請再送一次並帶 confirm_shrink。` }, 409);
        }
        spec.slug = slug;
